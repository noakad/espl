...
//parsediff("byte 33 -174 +230", &diff);//לא משנה מה "פארסדיף" מחזירה לי - ברגע שהשתמשתי בה, זה יוצר לי "סטראקט" -דיף ומאתחל לי את השדות להיות מה שביקשתי בקריאה לפונקצי זו. ככה שא"כ אני יכולה להשתמש ב"דיף" הזה ולפי השדות שלו לעשות את השינויים הנדרשים.עליי לעשות לולאה(שמשתמשת ב3 פונקציות ארבע פעמים) ובלולאה זו אני בעצם מבצעת את השינויים הנדרשים. לא לשכוח שעליי לבדוק שבאמת במקום שאני רוצה לבצע שינוי נמצא באמת הביית שציפיתי שיהיה (זה שמופיע אחרי המינוס) - ב"דיף" נמצא בשדה שנקרא אולד
//if(c1!=c2){
	//	printf("byte %lu -%u +%u \n", i, c1,c2);
